class Streebog:
    def __init__(self, Length):
        ...
        self.V0 = bytearray(64)

        if self.hash_length == 256:
            for i in range(64):
                self.V0[i] = 0x01

        if (self.hash_length != 256) and (self.hash_length != 512):
            raise Exception("Задана неправильная длина")

        self.Pi = bytearray([0xFC, 0xEE, 0xDD, 0x11, 0xCF, 0x6E, 0x31, 0x16, 0xFB, 0xC4, 0xFA, 0xDA, 0x23, 0xC5, 0x04, 0x4D,
        ...
        self.Tau = bytearray([0, 8, 16, 24, 32, 40, 48, 56,
    ...
    #Сложение в кольце вычетов по модулю 2 в степени 512
    def add_mod512(self, a, b):
        sum_ = 0
        res = bytearray(64)
    ...
    #Покомпонентное сложение по модулю 2 (xor)
    def xor(self, a, b):
        res = bytearray(64)
    ...
    def L(self, data):
        ...
        for i in range(8):
            ...
            for j in range(8):
                for k in range(8):
                    if (data[i * 8 + j] & (1 << (7 - k))) != 0:
                        temp ^= self.A[j * 8 + k]

            for j in range(8):
                res[i * 8 + j] = (temp & (0xff << (7 - j) * 8)) >> (7 - j) * 8

        return res

    #Формирование ключа К на этапах функции Е
    def K_evaluation(self, K, num):
        K = self.xor(K, self.C[num])
        K = self.S(K)
        K = self.P(K)
        K = self.L(K)
    ...
    def g(self, N, h, m):
        ...
        return self.xor(res, self.xor(h, m))
    ...
    def hash(self, M):
        ...
        #Создание массива 64 байт, в котором записано число 512(10)
        Number_512 = bytearray(64)
        Number_512[62] = 0x02

        #Создание массива 64 байт, в котором записано число 0
        Number_0 = bytearray(64)
        ...
        for i in range(1, length // 64 + 1):
            m = M[-i * 64 :][:64]  #Выделяем m по правилу M = M' || m
            h = self.g(N, h, m)
            N = self.add_mod512(N, Number_512)
            Sigma = self.add_mod512(Sigma, m)

        #Этап 3
        m = bytearray(64)
        m[64 - length % 64 - 1] = 0x01  #Дополнение единицей слева
        m[64 - length % 64 :] = M[: length % 64]  #Запись в m последней части из M

        #Создание массива длиной 64 байта, содержащего длину M на 3 этапе
        M_length_arr = bytearray(64)

        #Запись длины M на 3 этапе в байтовый массив (big endian)
        for j in range(2):
            M_length_arr[63 - j] = (length * 8 & 0xff << j * 8) >> j * 8

        h = self.g(N, h, m)
        N = self.add_mod512(N, M_length_arr)
        Sigma = self.add_mod512(Sigma, m)

        h = self.g(Number_0, h, N)

        h = self.g(Number_0, h, Sigma)

        if self.hash_length == 512:
            return h
        else:
            return h[:32]